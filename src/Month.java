...
public class Month {
    ...
    ArrayList<MonthRecord> monthT = new ArrayList<>();
    HashMap<Integer, Integer> profitOrders = new HashMap<>();
    HashMap<Integer, Integer> spendOrders = new HashMap<>();
    HashMap<Integer, ArrayList<MonthRecord>> hash = new HashMap<>();
    ...
    public void loadFile() {
        for (int month = 1; month <= 3; month++) {
            ...
            for (int j = 1; j < content.size(); j++) {
                ...
                MonthRecord monthTrans = new MonthRecord(name, isExpense, quantity, price);
    ...
    void getMaxProfitOrder(Integer month) {
        int maxOrder = 0;
        String maxOrderTitle = null;
        if (hash.containsKey(month)) {
            ArrayList<MonthRecord> freqs = hash.get(month);
            for (MonthRecord freq : freqs) {
                if (!freq.isExpense) {
        ...
        System.out.println("Самый прибыльный товар - " + maxOrderTitle + " и его стоимость: " + maxOrder + " рублей");
    }

    void getMaxSpendOrder(Integer month) {
        int maxSpend = 0;
        ...
        System.out.println("Самая большая трата - " + maxSpendTitle + " и сумма траты: " + maxSpend + " рублей");
    }

    void getIncomeAndSpend() {
        int income = 0;
        int spend = 0;
        for (Integer month : hash.keySet()) {
            ArrayList<MonthRecord> transaction = hash.get(month);
            for (MonthRecord sale : transaction) {
